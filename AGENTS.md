...
```
┌─────────────────────────────────────────────────────────────┐
│  窗口 A: 开发 (Coding Agent)                                │
│  - 实现功能                                                  │
│  - 完成后说 "功能完成，请求验收"                              │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  窗口 B: 验收 (Verification Agent - 全新对话/不同模型)        │
...
1. **开发完成后**：Coding Agent 说 "功能完成，请求验收"
2. **验收 Agent**：在全新对话中使用 `ultrabrain` 或 `deep` category 验证
3. **自动更新**：验收通过后自动更新 feature_list.json (passes: true)
4. **提交代码**：回到原窗口提交 git
...
验证通过后，请执行：
1. 更新 feature_list.json 中该功能的 passes 为 true
2. 说 "验收通过"
```
